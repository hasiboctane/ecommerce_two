<div class="w-full max-w-[85rem] py-16 px-4 sm:px-6 lg:px-8 mx-auto">
    <h1 class="text-3xl font-bold text-slate-500 pt-4">Order Details</h1>
        ...
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
            <div class="p-4 md:p-5 flex gap-x-4">
                ...
                <div class="grow">
                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        <span
                            class="bg-{{ $order->status == 'cancelled' ? 'red' : ($order->status == 'processing' ? 'blue' : ($order->status == 'shipped' ? 'green' : ($order->status == 'delivered' ? 'green' : 'gray'))) }}-500 py-1 px-3 rounded text-white shadow">{{ $order->status }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
            <div class="p-4 md:p-5 flex gap-x-4">
                ...
                <div class="grow">
                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        <span
                            class="bg-{{ $order->payment_status == 'paid' ? 'green' : ($order->payment_status == 'pending' ? 'orange' : 'red') }}-500 py-1 px-3 rounded text-white shadow">{{ $order->payment_status }}</span>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="flex items-center">
                                        @if ($item->product->images !== null)
                                            <img src="{{ url('storage', $item->product->images[0]) }}"
                                                alt="{{ $item->product->name }}" class="h-16 w-16 mr-4">
                                        @else
                                            <img src="{{ asset('images/null.jpg') }}" alt="{{ $item->product->name }}"
        ...
        <div class="md:w-1/4">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="flex justify-between mb-2">
                    ...
                    <span class="font-semibold">{{ Number::currency($order->grand_total, 'BDT') }}</span>
                </div>
