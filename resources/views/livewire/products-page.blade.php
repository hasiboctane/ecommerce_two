                ...
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                                ...
                                <li class="mb-4" wire:key='{{ $category->id }}'>
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" wire:model.live='selected_categories'
                                            class="w-4 h-4 mr-2" id="{{ $category->slug }}" value="{{ $category->id }}">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="text-xl font-bold text-rose-600">
                            {{ Number::currency($price_range, 'BDT') }}
                            {{-- {{ Number::currency(3000, 'BDT') }} --}}
                        </div>
                        <div>
                            <input type="range"
                                class="w-full h-1 mb-4 bg-rose-100 rounded appearance-none cursor-pointer"
                                max="50000" value="{{ $price_range }}" step="500"
                                wire:model.live='price_range'>
                            <div class="flex justify-between ">
                                <span class="inline-block text-md font-bold text-slate-600 ">500</span>
                                <span class="inline-block text-md font-bold text-slate-600 ">50000</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-wrap items-center ">
                                ...
                                <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key='{{ $product->id }}'>
                                    <div class="border border-gray-300 dark:border-gray-700">
                                        <div class="relative bg-gray-200">
                                            <a href="{{ route('product.show', ['parent_category' => $product->parentCategory->slug, 'category' => $product->category->slug, 'slug' => $product->slug]) }}"
                                                class="">
                                        ...
                                        <div class="p-3 ">
                                            ...
                                            <p class="text-lg ">
                                                <span
                                                    class="text-green-600 dark:text-green-600">{{ Number::currency($product->price, 'BDT') }}</span>
                                            </p>
                                        </div>
                                        <div
                        ...
                        @else
                            <div class="flex justify-start items-center mx-auto">
                                <h1 class="text-3xl font-bold text-gray-500">No Product Found</h1>
                            </div>
                        @endif
